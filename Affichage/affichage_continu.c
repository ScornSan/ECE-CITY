...
int bouton(BITMAP* bitmap, int x, int y) {
    if (mouse_b & 1 && mouse_x > x && mouse_x < x + bitmap->w && mouse_y > y && mouse_y < y + bitmap->h) {
        return 2;
    }
    else if (mouse_x > x && mouse_x < x + bitmap->w && mouse_y > y && mouse_y < y + bitmap->h) {
        return 1;
    }
    return 0;
}
...
void affichage_hud(t_affichage* hud, BITMAP* buffer, t_joueur* joueur){
    ...
    textprintf_ex(buffer,font,205,16+24,makecol(255,255,255),-1,"%d", joueur->habitants);

    //Constructions
    if (bouton(hud->construct, 10, 650)){
        masked_blit(hud->bg, buffer, 0, 0, 10, 650, SCREEN_W, SCREEN_H);
