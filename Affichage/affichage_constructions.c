...
void boucle_affichage(int i, int j, int hauteur, int longueur, t_plateau *plateau, BITMAP *buffer, int position_x,
                      int position_y, int deplacement, int type, int indice) {
        ...
        case 2 :   /// habitations 3x3 (Cabane maison chantier ruine)
            if (plateau->niveau == 0) {
                if (plateau->habitations[indice]->niveau <= 3) {
                    masked_blit(plateau->habitations[indice]->style[plateau->habitations[indice]->niveau], buffer, 0, 0,
                                plateau->matrice[i + position_x][j + position_y].x_bloc + 2 * deplacement -
                                plateau->matrice[i][j].b_element->w / 2 - plateau->screenx,
                                plateau->matrice[i + position_x][j + position_y].y_bloc + deplacement -
                                plateau->matrice[i][j].b_element->h / 2 - plateau->screeny, SCREEN_W, SCREEN_H);
                    break;
                }
                else if (plateau->habitations[indice]->niveau == 10) { //Ruine
                    masked_blit(plateau->habitations[indice]->style[0], buffer, 0, 0,
                                plateau->matrice[i + position_x][j + position_y].x_bloc + 2 * deplacement -
                                plateau->matrice[i][j].b_element->w / 2 - plateau->screenx,
                                plateau->matrice[i + position_x][j + position_y].y_bloc + deplacement -
                                plateau->matrice[i][j].b_element->h / 2 - plateau->screeny, SCREEN_W, SCREEN_H);
                    ...
                    masked_blit(plateau->habitations[indice]->style[plateau->habitations[indice]->niveau], buffer, 0, 0,
                                plateau->matrice[i + position_x][j + position_y].x_bloc + 2 * deplacement -
                                plateau->matrice[i][j].b_element->w / 2 - plateau->screenx + 2,
                                plateau->matrice[i + position_x][j + position_y].y_bloc + deplacement -
                                plateau->matrice[i][j].b_element->h / 2 - plateau->screeny - 48, SCREEN_W, SCREEN_H);
                    break;
                ...
                if (plateau->habitations[indice]->niveau <= 3) {
                    ...
                    textprintf_ex(buffer, font,
                                  20 + plateau->matrice[i + position_x][j + position_y].x_bloc + 2 * deplacement -
                                  plateau->matrice[i][j].b_element->w / 2 - plateau->screenx,
                                  plateau->matrice[i + position_x][j + position_y].y_bloc + deplacement -
                                  plateau->matrice[i][j].b_element->h / 2 - plateau->screeny - 10, makecol(0, 100, 255),
                                  -1, "%d/1000", plateau->habitations[indice]->quantite_eau);
                    ...
                    textprintf_ex(buffer, font,
                                  ...
                                  plateau->matrice[i][j].b_element->w / 2 - plateau->screenx,
                                  plateau->matrice[i + position_x][j + position_y].y_bloc + deplacement -
                                  plateau->matrice[i][j].b_element->h / 2 - plateau->screeny - 50, makecol(0, 100, 255),
                                  -1, "%d/1000", plateau->habitations[indice]->quantite_eau);
                    break;
                } else if (plateau->habitations[indice]->niveau == 5) { // GRATTE CIEL
                    masked_blit(plateau->habitations[indice]->style_noir[plateau->habitations[indice]->niveau], buffer,
                    ...
                    textprintf_ex(buffer, font,
                                  ...
                                  plateau->matrice[i][j].b_element->w / 2 - plateau->screenx,
                                  plateau->matrice[i + position_x][j + position_y].y_bloc + deplacement -
                                  plateau->matrice[i][j].b_element->h / 2 - plateau->screeny - 70, makecol(0, 100, 255),
                                  -1, "%d/5000", plateau->habitations[indice]->quantite_eau);
                    break;
                }
                ...
                case 3:   /// batiments 4x6
                    if (plateau->niveau == 0) {
                        masked_blit(plateau->batiments[indice]->style[plateau->batiments[indice]->element - 1], buffer,
                                    0, 0, plateau->matrice[i + position_x][j + position_y].x_bloc + 2 * deplacement -
                                          plateau->matrice[i][j].b_element->w / 2 - plateau->screenx,
                                    plateau->matrice[i + position_x][j + position_y].y_bloc + deplacement -
                                    plateau->matrice[i][j].b_element->h / 2 - plateau->screeny, SCREEN_W, SCREEN_H);
                    } else if (plateau->niveau == 1) {
                        masked_blit(plateau->batiments[indice]->style_noir[plateau->batiments[indice]->element - 1],
                                    buffer, 0, 0,
                                    plateau->matrice[i + position_x][j + position_y].x_bloc + 2 * deplacement -
                                    plateau->matrice[i][j].b_element->w / 2 - plateau->screenx,
                                    plateau->matrice[i + position_x][j + position_y].y_bloc + deplacement -
                                    plateau->matrice[i][j].b_element->h / 2 - plateau->screeny, SCREEN_W, SCREEN_H);
                    } else if (plateau->niveau == 2) {
                        masked_blit(plateau->batiments[indice]->style_noir[plateau->batiments[indice]->element - 1],
                                    buffer, 0, 0,
                                    plateau->matrice[i + position_x][j + position_y].x_bloc + 2 * deplacement -
                                    plateau->matrice[i][j].b_element->w / 2 - plateau->screenx,
...
void affichage_construction(t_affichage *hud, BITMAP *buffer, t_joueur *joueur, t_plateau *plateau) {
    ...
    for (int i = 0; i < 35; i++) {
        //printf("\n");
        for (int j = 0; j < 45; j++) {
            if (plateau->matrice[i][j].affiche !=
                1) { ///pour voir si on affiche pas deux fois la meme maison en allant sur le bloc suivant
                switch (plateau->matrice[i][j].element) {
                    case 0 :
                        break;

                    case CHATEAU_EAU:
                        boucle_affichage(i, j, 4, 6, plateau, buffer, 0, 3, 7, 3,
                    ...
                    case BANQUE:
                        ...
                        break;
                    case CHANTIER:
                        boucle_affichage(i, j, 3, 3, plateau, buffer, 1, 1, 0, 2,
                                         plateau->matrice[i][j].id_element);
                        break;
                    case CABANE:
                        boucle_affichage(i, j, 3, 3, plateau, buffer, 1, 1, 0, 2,
    ...
    for (int x = 0; x < plateau->indice_tab_habitations; x++) // on parcours notre tableau d'habitation
    {
        if ((int) time(NULL) - plateau->habitations[x]->timer >= TIME_CYCLE && plateau->indice_tab_habitations !=0 && plateau->habitations[x]->niveau < GRATTECIEL) // on verifie si le cycle de 15 secondes d'une des habitations est fini
        {
            ...
            printf("%d INCENDIE: %d\n", x, plateau->habitations[x]->incendie);
            plateau->habitations[x]->timer = time(NULL);
            maj_habitation(x, plateau, joueur, buffer, hud);
        } else {
            rectfill(buffer, 993, 723, 993 - (int) ((time(NULL)) - plateau->habitations[x]->timer) * 11, 746,
                     makecol(255, 0, 0)); // barre de temps
        }
    }
        ...
        for (int j = 0; j < 45; j++) {
            ...
            if (plateau->matrice[i][j].affiche !=
                ...
                switch (plateau->matrice[i][j].element) {
                    ...
                    case ROUTES:
                        plateau->matrice[i][j].affiche = 1;
                        masked_blit(plateau->matrice[i][j].b_element, buffer, 0, 0,
                                    plateau->matrice[i][j].x_bloc - plateau->matrice[i][j].b_element->w / 2 -
                                    plateau->screenx,
                                    plateau->matrice[i][j].y_bloc - plateau->matrice[i][j].b_element->h / 2 -
                                    plateau->screeny, SCREEN_W, SCREEN_H);
                        break;
                    case TVAGUE:
                        plateau->matrice[i][j].affiche = 1;
                        dessin_bloc_unique(buffer, i, j, plateau, 119, 136, 153);
