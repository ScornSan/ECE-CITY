    ...
    while(!clic){
        ...
        if(indice == 4){        // si c'est une maison
            ...
            if(mouse_b&1 && plateau->matrice[plateau->lig_mouse][plateau->col_mouse].element == TVAGUE_CP) {
                ...
                while (difftime(time(NULL), seconds) < 1)
                {
                    clear_bitmap(buffer);
                    affichage_elements(hud, buffer, joueur, plateau);
                    textprintf_ex(buffer, font, mouse_x - 20, mouse_y, makecol(255,0,0),-1, "Impossible !");
            ...
            for(int i = 0; i< 4; i++){
                if (i == indice){
                    masked_blit(hud->construction[indice][2], buffer, 0, 0, plateau->matrice[plateau->lig_mouse][plateau->col_mouse+1].x_bloc - hud->construction[indice][2]->w/2, plateau->matrice[plateau->lig_mouse][plateau->col_mouse+1].y_bloc - hud->construction[indice][2]->h/2, SCREEN_W, SCREEN_H);
                }
            }
            if(mouse_b&1 && plateau->matrice[plateau->lig_mouse][plateau->col_mouse].element == TVAGUE_CP) {
